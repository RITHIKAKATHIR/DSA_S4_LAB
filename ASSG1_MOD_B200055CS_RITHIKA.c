...
struct node_bt* node_search_bt(struct node_bt* search,int k){
    if(search==NULL) return NULL ;
    if(search->value==k) return search;

    struct node_bt* y=node_search_bt(search->leftchild,k);
    if(y!=NULL) return y;

    
         
    struct node_bt* y_1=node_search_bt(search->leftchild,k);
...
void new_year_invite(struct node_bt* root, struct node_bt* present, struct array* A, int k){
     if(root==NULL || k<0) return ;
     else{
         if(k==0){ /*printf("%d",present->value);*/ 
         A->A[(A->j)++]=present->value; 
         return ;}
         
         new_year_invite(root,root->leftchild,A,k-1) ; 
         new_year_invite(root,root->rightchild,A,k-1) ; 
         
     } 
}

/*void to_search_ancestor(struct node_bt* root, struct node_bt* present, struct array* A, int k){
    if(root==present) return ;
    struct node_bt* y;
    y=(struct node_bt*)malloc(sizeof(struct node_bt));
    y=root;
    struct node_bt* x;
    x=(struct node_bt*)malloc(sizeof(struct node_bt));
    while(y!=NULL && y!=present){
       x=y;
       y=y->leftchild;
    }
    if(y==present) new_year_invite(root,x,A,k-1);
    else{y=root;
        while(y!=NULL && y!=present){
       x=y;
       y=y->rightchild;
...
int main()
{
   ...
   struct node_bt* node_present=node_search_bt(root,s);
   //printf("new_node: %d", node_present->value);
   if(node_present!=NULL){
   struct array* a;
   a=(struct array*)malloc(sizeof(struct array));
   a->j=0;
   new_year_invite(root,node_present,a,k);
   int sum=0;
    for(int i=0;i<(a->j);i++){
       sum=sum+(a->A[i]);
   } 
