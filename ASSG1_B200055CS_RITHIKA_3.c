...
int MINIMUM(struct node_bt* a){
    if(a==NULL)return -9999999;
    while(a->leftchild!=NULL && a->leftchild->value!=-9999999){
        a=a->leftchild;
    }
    return a->value;
}

int MAXIMUM(struct node_bt* a){
    if(a==NULL)return 9999999;
    while(a->rightchild!=NULL && a->rightchild->value!=-9999999){
        a=a->rightchild;
    }
    return a->value;
}
...
void inorder(struct node_bt* root, int* arr){
    if(root!=NULL && root->value!=-9999999){
...
int isBST(struct node_bt* root){
    int a=1;
    int* arr2;
    arr2=(int*)malloc(1000000*sizeof(int));
    // int* arr3;
    // arr3=(int*)malloc(100000*sizeof(int));
    // int cnt=0;
    cnt=0;
    inorder(root, arr2);
    for(int i=1;i<cnt;i++){
...
int rithika(struct node_bt* root){
    int a,b;
    a=isBST(root->leftchild);
	//printf("a is: %d ",a);
    b=isBST(root->rightchild);
	//printf("b is:%d ",b);
    int ans=0;
    if(a!=1 || MAXIMUM(root->leftchild)>root->value)ans++;
    if(b!=1 || MINIMUM(root->rightchild)<root->value)ans++;
    return ans;
}
